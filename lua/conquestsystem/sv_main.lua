...
ConquestSystem.Points = ConquestSystem.Points or {}
...
function ConquestSystem.GetContestedPoint(ply)
    ...
    for k, v in pairs(ConquestSystem.Points) do
        ...
        if dist < v.Radius then return v end
    end
...
function ConquestSystem.GetContestingTeam(point)
    for k, v in pairs(point.ContestedBy) do
        return k
    end
end
...
function ConquestSystem.CheckPositions()
    ...
    for k, v in pairs(ConquestSystem.Points) do
        ...
        -- point being fought over.
        if numContestingTeams > 1 then
            -- reset capturing time and owner.
            v.Capturing = nil
        elseif numContestingTeams == 1 then
            -- point with one team in it.
            ...
            if teamContesting ~= v.Owner then
                -- this point previously was fought over.
                if v.Capturing == nil then
        ...
        elseif numContestingTeams == 0 then
            -- point with no one in it.
            v.Capturing = nil
        end
    end
...
function ConquestSystem.SaveToSQL()
    ...
    for k, v in pairs(ConquestSystem.Points) do
        ...
        if v.CategoryEnabled then
            isEnabled = 1
        end

        -- does this point exist in the table
        local queryResult = sql.Query("SELECT name FROM conq_points_120 WHERE tag = " .. sql.SQLStr(v.Tag) .. " AND mapname = " .. sql.SQLStr(game.GetMap()))
...
function ConquestSystem.LoadFromSQL()
    ...
    for k, v in pairs(queryResult) do
        ...
        if tonumber(v.categoryenabled) == 1 then
            isEnabled = true
        end
...
concommand.Add("ConquestSystem_Sv_CreatePoint", function(ply, cmd, args)
    ...
    local shape = args[5]
    args[6] = args[6] and string.Replace(args[6], "'", "\"") or ""
    local disallowedTeams = util.JSONToTable(args[6])
    local isEnabled = false

    if tonumber(categoryEnabled) == 1 then
        isEnabled = true or disallowedTeams ~= nil
    end
...
timer.Simple(4, function()
    WaitCallback(function() return (sql.TableExists("conq_points_120") and sql.TableExists("conq_points_teams_120")) end, function()
...
hook.Add("PlayerSpawn", "CS_PLAYSPAWN", function(ply)
    timer.Simple(0.1, function()
        if ConquestSystem.Config.Spawning.Menu and ply.SpawnPosition ~= nil then
