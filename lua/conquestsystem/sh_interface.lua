function ConquestSystem.GetPointColour(point)
    if point.CategoryEnabled then
        if ConquestSystem.Config.DarkRP then
            if LocalPlayer():getJobTable().command == point.Owner then return LocalPlayer():getJobTable().color end

            return Color(255, 0, 0, 255)
        else
            -- find that team
            for k, v in pairs(team.GetAllTeams()) do
                if v.Name == point.Owner then return v.Color end
            end
...
function ConquestSystem.GetTeamShape(point, ply)
    if ConquestSystem.Config.TeamShapes[ConquestSystem.GetTeamName(point, ply)] ~= nil then return ConquestSystem.Config.TeamShapes[ConquestSystem.GetTeamName(point, ply)] end
...
function ConquestSystem.FindTeamByCommand(command)
    for k, v in pairs(RPExtraTeams) do
        if v.command == command then return v end
    end
end
...
function ConquestSystem.GetPlayerOwnedPoints(ply)
    ...
    if SERVER then
        points = ConquestSystem.Points
    else
        points = ConquestSystem.CapturePoints
    end
