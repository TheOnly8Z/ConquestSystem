...
function ConquestSystem.RenderHUDIcon(point, x, y, dist, isVisible)
	...
	-- Do a check to see if the player is looking at the icon, which tells us to hide it
	local thres = 0.05
	if math.abs(ScrW() / 2 - x) <= ScrW() * thres or math.abs(ScrH() / 2 - y) <= ScrH() * thres then
		alpha = (math.abs(ScrW() / 2 - x) / (ScrW() * thres) + math.abs(ScrH() / 2 - y) / (ScrH() * thres)) / 2
    ...
    draw.SimpleTextOutlined(point.Tag, ConquestSystem.Config.Fonts.Tag, x, tagy, ConquestSystem.Config.Colours.TagFill, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, Color(0, 0, 0, 255))

    if isVisible or (not isVisible and atPoint) then
        draw.SimpleTextOutlined(point.Name, ConquestSystem.Config.Fonts.Name, x, y + (ConquestSystem.Config.Sizes.IconForegroundShapeRadius / 2) + (ConquestSystem.Config.Positioning.TextPaddingTop), ConquestSystem.Config.Colours.NameFill, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, Color(0, 0, 0, 255))
        draw.SimpleTextOutlined(math.Round(dist / 100) .. "m", ConquestSystem.Config.Fonts.Distance, x, y + (ConquestSystem.Config.Sizes.IconForegroundShapeRadius / 2) + (ConquestSystem.Config.Positioning.TextPaddingTop) + (ConquestSystem.Config.Positioning.TextSpacing * 2), ConquestSystem.Config.Colours.DistanceFill, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, Color(0, 0, 0, 255))
    end
